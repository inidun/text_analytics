...
def display_gui(corpus_folder, container=None):
    ...
    candidate_corpus_filenames = [
        os.path.basename(x) for x in glob.glob(os.path.join(corpus_folder, "*.zip"))
    ]

    year_range = [1920, 2020]
    POS_LIST = ['', 'ADJ', 'ADP', 'ADV', 'AUX', 'CONJ', 'CCONJ', 'DET', 'INTJ', 'NOUN', 'NUM', 'PART', 'PRON', 'PROPN', 'PUNCT', 'SCONJ', 'SYM', 'VERB', 'X', 'EOL', 'SPACE']
    ...
    gui = types.SimpleNamespace(
        corpus_filename=widgets.Dropdown(
            ...
            value=None
            if not candidate_corpus_filenames
            else candidate_corpus_filenames[-1],
        ...
        filter_stops=widgets.Checkbox(
            value=True,
            description='Filter stops',
            disabled=False,
            indent=False
        ),
        filter_punct=widgets.Checkbox(
            value=True,
            description='Filter puncts',
            disabled=False,
            indent=False
        ),
        filter_nums=widgets.Checkbox(
            value=True,
    ...
    tab_widget.children = [corpus_widget, extra_args_widget]
    tab_titles = ['Annotate','Prepare'] 
